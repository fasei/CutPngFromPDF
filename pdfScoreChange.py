...
Created on 2019年9月7日 11:01:37
  ...
  1  优化尾部正则匹配规则
  1  去除无效的截图（第一张图匹配为尾部）
  1  优化两图并列时，一个来源的情况，截成一张大图（内部可能还包含多个表）
  1  表头和表尾中间的内容高度小于20的会忽略
@author: 王超
"""
            ...
            while i0 <= len(loc_top) - 1 and j0 <= len(loc_bottom) - 1:
                ...
                if is_binglie == 0:  # 只有一行的时候
                    ...
                    if is_binglie_laiyuan == 2:  # 并列来源
                        ...
                        # top1 = [(0, loc_top[i0][0][1], loc_top[i0 + 1][0][0], loc_top[i0][0][3]), loc_top[i0][1]]
                        # locname.append([top1, 0])
                        # bottom1 = [(0, loc_bottom[j0][0][1], loc_top[i0 + 1][0][0], loc_bottom[j0][0][3]),loc_bottom[j0][1]]
                        # locname.append([bottom1, 1])
                        #
                        # top2 = [(loc_top[i0 + 1][0][0], loc_top[i0 + 1][0][1], canvas_size[2], loc_top[i0 + 1][0][3]),loc_top[i0 + 1][1]]
                        # locname.append([top2, 0])
                        # bottom2 = [(loc_top[i0 + 1][0][0], loc_bottom[j0][0][1], canvas_size[2], loc_bottom[j0][0][3]),loc_bottom[j0][1]]
                        # locname.append([bottom2, 1])
    ...
    for files in os.walk(file_dir):
        ...
        for pdf_name in files[2]:
            if pdf_name[-3:] == 'pdf':
                ...
                # 复制文件方便查看
                copyfile(pdf_path, cropped_pic_path + '\\' + pdf_name)
                ...
                try:
                    ...
                    for i in range(page_count):
                        try:
                            # test = GetPic(pdf_path)
                            tmp, text_total, log_total, topNumber, bottomNumber, hasFind = test.main(pic_path,
                                                                                                     cropped_pic_path,
                                                                                                     pgn=i, tmp=tmp)
...
if __name__ == '__main__':
    ...
    # file_dir = 'd:\\pdfreport\\upfile\\temp\\'
    # file_dir='d:\\pdfreport\\upfile\\hengpdf\\'

    file_dir = 'd:\\pdf\\pdffile\\'

    # start='d:\\pdfreport\\t\\'
    # start = 'd:\\pdfreport\\self\\'
    # start = 'd:\\pdfreport\\heng\\'
    start = 'd:\\pdf\\pdfresult\\'
    # start = 'd:\\pdfreport\\test\\'

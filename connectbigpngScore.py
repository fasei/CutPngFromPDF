            ...
            if pdf_name[-3:] == 'pdf':
                ...
                try:
                    ...
                    for i in range(1):
                        try:
                            # test = GetPic(pdf_path)
                            tmp, text_total, log_total, topNumber, bottomNumber = test.main(pic_path, cropped_pic_path,
                                                                                            pgn=i, tmp=tmp)
...
# 定义图像拼接函数
def image_compose(filePath, count):
    '''
    :param filePath:  png图片所在的路径
    :param count:  拼接的图片数量
    :return: 新建的大图片
    '''
    ...
    for i in range(count):
        from_image = Image.open(filePath + '\\' + str(i) + '.png')
        list.append(from_image)

    htitch = np.vstack(list)
    im = Image.fromarray(htitch)
    im.save(filePath + '\\' + 'big.png')
...
if __name__ == '__main__':
    ...
    # image_compose('D:\\pdfreport\\t\\pic_path\\38890318',6)
